...
class Board:
    ...
    def place_pieces(self): # will use this x and y to position the graphics also 
        self.board.clear()
        self.whites.clear()
        self.blacks.clear()
    ...
    def save_pieces(self): #save the pos of black and white in lists 
        for i in range(8):
            for j in range(8):
    ...
    def has_opponent(self, piece, x, y): # checks if it has opponent
    ...
    def has_friend(self, piece, x, y): #furendssssssssssss
        if not self.is_valid_move(x, y):
            return False
    ...
    @staticmethod
    def is_valid_move(x, y): #out of bound
        return 0 <= x < 8 and 0 <= y < 8

    def has_empty_block(self, x, y): # empty block 
        if not self.is_valid_move(x, y):
            return False
    ...
    def king_is_threatened(self, color, move=None): #check checker xD 
        if color == 'white':
            enemies = self.blacks
            king = self.whiteKing
        else:
    ...
    def calculate_piece_activity(self, row, col, color): #if the pieces are in central position give them more poiints
        activity_points = 0
        # Add points based on piece activity (e.g., centralization)
